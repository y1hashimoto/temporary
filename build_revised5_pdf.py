...
story.append(Paragraph('rev.1：侵入経路は“取引先からの指摘”で判明する形に修正／本編 約7分＋解説',subt))
...
bar('＃4　異変と、取引先からの一報（侵入経路は“指摘されて”分かる）　2:30–3:40')
A('○数週間後。木川精機で受発注が不調。多田、念のためVPNのアクセス記録を確認。')
L('多田','……深夜に、見慣れない海外からのログイン。管理者権限まで使われてる。うちのVPN、入られたかもしれない')
L('鈴木','うちが、やられたのか？')
L('多田','まだ分かりません。でも、この入口は――<b>MFAを掛けていなかった、あの鍵</b>だ')
A('○そこへ、取引先・メイユー自動車（架空）から連絡。基幹が暗号化され操業が長期停止しているという。')
L('メイユー（情シス）','（OFF）木川さん、緊急で。弊社が不正アクセスを受けて調査中なんですが……<b>侵入の起点が、御社との接続経由だった可能性が高い</b>。ログの保全と、調査へのご協力をお願いします')
L('多田','（息を呑む）……やはり、そこから')
A('○後日。両社とフォレンジック調査、JPCERTの連携で経路が裏付けられる。')
L('多田','（社長に）裏が取れました。<b>盗まれた、うちのVPNの認証情報</b>で、メイユーさんへ入られていた。……<b>うちが、入口でした</b>')
L('穴のM','自分の穴は、自分では気づきにくい。教えてくれたのは――つながった、相手だった')
NB('侵入経路は木川単独では特定できない。取引先の調査・フォレンジック・JPCERT連携で判明し“指摘される”のが現実的。＝自社だけでは気づけない＝普段の備えと相談先が要る、という教訓に。')
